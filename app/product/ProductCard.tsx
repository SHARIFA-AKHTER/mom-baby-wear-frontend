...
export default function ProductCard({ product }: Props) {
  ...
  const isFavorite = items.some((item: any) => item.id === product.id);
  ...
  return (
    <div className="group rounded-xl border bg-white shadow-sm hover:shadow-lg transition flex flex-col overflow-hidden relative">
      ...
      <button
        ...
        disabled={isPending}
        className="absolute top-3 right-3 z-10 p-2 bg-white/80 backdrop-blur-sm rounded-full shadow-md hover:bg-white transition-all group/heart"
      >
        {isPending ? (
