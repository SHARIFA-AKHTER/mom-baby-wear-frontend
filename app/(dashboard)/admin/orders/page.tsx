...
export default function AdminOrders() {
  ...
  const updateStatus = async (id: string, status: OrderStatus) => {
    try {
      ...
      if (Array.isArray(ordersData)) {
        ...
        setOrdersData({ ...ordersData, result: updatedList });
      }
    } catch (error) {
      alert("Failed to update status");
    }
  ...
  const deleteOrder = async (id: string) => {
    ...
    try {
      ...
      if (Array.isArray(ordersData)) {
        setOrdersData(filteredList);
      } else {
        setOrdersData({ ...ordersData, result: filteredList });
      }
    } catch (error) {
      alert("Failed to delete order");
    }
        ...
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-gray-50">
            {ordersList.map((order) => (
              <tr key={order.id} className="hover:bg-gray-50/50 transition-colors">
                <td className="p-4 font-medium text-gray-800">{order.user?.email || 'N/A'}</td>
                <td className="p-4 text-gray-500">
                  {new Date(order.createdAt).toLocaleDateString("en-GB", {
                    day: "2-digit",
                    month: "short",
        ...
        {ordersList.map((order) => (
          <div key={order.id} className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
            <div className="flex justify-between items-start mb-4">
              <div className="max-w-[70%]">
                <p className="text-xs text-gray-400 uppercase font-bold tracking-wider">Customer</p>
                <p className="font-bold text-gray-800 truncate">{order.user?.email || 'N/A'}</p>
              </div>
              <span className={`text-[10px] font-black px-2 py-1 rounded-md border ${statusStyles[order.status]}`}>
                {order.status}
              </span>
            ...
            <div className="flex items-center gap-3 pt-4 border-t border-gray-50">
              <select
                value={order.status}
                onChange={(e) => updateStatus(order.id, e.target.value as OrderStatus)}
                className="flex-1 bg-gray-50 border border-gray-200 text-sm rounded-xl px-3 py-2 outline-none"
              >
                {Object.keys(statusStyles).map((status) => (
                  <option key={status} value={status}>{status}</option>
                ))}
