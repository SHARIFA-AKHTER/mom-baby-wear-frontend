    ...
    <header className="bg-white dark:bg-gray-950 border-b dark:border-gray-800 sticky top-0 z-50 shadow-sm transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between gap-2">
        ...
        <nav className="hidden lg:flex items-center gap-4 xl:gap-8 text-[14px] font-medium text-gray-600 dark:text-gray-300">
          ...
          {/* New Desktop Links */}
          <LinkNext href="/offers" className="flex items-center gap-1 hover:text-pink-600 transition-colors">
            <Tag size={16} className="text-pink-500" /> Offers
          </LinkNext>

          <LinkNext href="/about" className="hover:text-pink-600 transition-colors">
            About Us
          </LinkNext>
        </nav>
      ...
      {/* Mobile Sidebar */}
      {open && (
        <div className="lg:hidden fixed inset-0 top-16 bg-white dark:bg-gray-950 z-50 border-t dark:border-gray-800 p-5 animate-in slide-in-from-right w-full overflow-y-auto">
          <div className="space-y-3 pb-20">
            <LinkNext onClick={() => setOpen(false)} href="/products" className="block p-4 bg-gray-50 dark:bg-gray-900 dark:text-gray-200 rounded-xl font-semibold">
              Products
            </LinkNext>

            {/* Responsive Offers & About for Mobile */}
            <div className="grid grid-cols-2 gap-3">
              <LinkNext onClick={() => setOpen(false)} href="/offers" className="flex items-center justify-center gap-2 p-4 bg-pink-50 dark:bg-pink-900/10 text-pink-600 dark:text-pink-400 rounded-xl font-bold">
                <Tag size={18} /> Offers
              </LinkNext>
              <LinkNext onClick={() => setOpen(false)} href="/about" className="flex items-center justify-center gap-2 p-4 bg-blue-50 dark:bg-blue-900/10 text-blue-600 dark:text-blue-400 rounded-xl font-bold">
                <Info size={18} /> About
              </LinkNext>
            </div>
